...
Class Action {
	...
	function login(){
		extract($_POST);
		$stmt = $this->db->prepare("SELECT *, CONCAT(lastname, ', ', firstname, ' ', middlename) as name FROM users WHERE email = ? AND type = 1");
		...
		if($result->num_rows > 0){
			...
			if (password_verify($password, $user['password'])) {
				$valid = true;
			} elseif (strlen($user['password']) == 32 && !preg_match('/^\$2y\$/', $user['password']) && md5($password) == $user['password']) {
				// Legacy MD5 password, rehash it
	...
	function login2(){
		extract($_POST);
		$stmt = $this->db->prepare("SELECT *, CONCAT(lastname, ', ', firstname, ' ', middlename) as name FROM users WHERE email = ? AND type = 2");
		$stmt->bind_param("s", $email);
		$stmt->execute();
		$result = $stmt->get_result();
		if($result->num_rows > 0){
	...
	function save_user(){
		...
		$stmt_check = $this->db->prepare("SELECT * FROM users WHERE email = ? " . (!empty($id) ? "AND id != ?" : ""));
		if(!empty($id)){
		...
		if(empty($id)){
			...
			$stmt = $this->db->prepare("INSERT INTO users (" . implode(',', $fields) . ") VALUES ($placeholders)");
